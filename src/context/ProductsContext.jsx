...
export const ProductsProvider = ({ children }) => {
  ...
  useEffect(() => {
    const cargarProductos = async () => {
      try {
        const respuesta = await fetch(`https://69248ada3ad095fb8474a1f2.mockapi.io/api/productos/`);
  ...
  const editarProducto = async (productoActualizado) => {
    try {
      const respuesta = await fetch(`https://69248ada3ad095fb8474a1f2.mockapi.io/api/productos/${productoActualizado.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
